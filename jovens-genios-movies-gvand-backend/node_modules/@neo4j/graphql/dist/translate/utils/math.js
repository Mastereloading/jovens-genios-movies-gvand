...
exports.buildMathStatements = exports.mathDescriptorBuilder = exports.matchMathField = exports.MATH_FIELD_REGX = void 0;
...
exports.MATH_FIELD_REGX = /(?<propertyName>\w*)(?<operatorName>_INCREMENT|_DECREMENT|_ADD|_SUBTRACT|_DIVIDE|_MULTIPLY)\b/;
// Returns True in case of a valid match and the potential match.
function matchMathField(graphQLFieldName) {
    const mathFieldMatch = graphQLFieldName.match(exports.MATH_FIELD_REGX);
...
function buildMathStatements(mathDescriptor, scope, withVars, param) {
    ...
    statements.push(`CALL apoc.util.validate(${scope}.${mathDescriptor.dbName} IS NULL, 'Cannot %s %s to Nan', ["${mathDescriptor.operationName}", $${param}])`);
    ...
    if (mathDescriptor.graphQLType === "Int" || mathDescriptor.graphQLType === "BigInt") {
        statements.push(`CALL apoc.util.validate((${scope}.${mathDescriptor.dbName} ${mathDescriptor.operationSymbol} $${param}) % 1 <> 0, 'Type Mismatch: Value returned from operator %s does not match: %s', ["${mathDescriptor.operationName}", "${mathDescriptor.graphQLType}"])`);
    }
