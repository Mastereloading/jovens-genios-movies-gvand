...
function aggregate({ inputValue, nodeOrRelationship, chainStr, variable, }) {
    const aggregations = [];
    let withStrs = [];
    let params = {};
    Object.entries(inputValue).forEach(([key, value]) => {
        ...
        const field = [...nodeOrRelationship.primitiveFields, ...nodeOrRelationship.temporalFields].find((f) => logicalOperators.some((op) => key.split(`_${op}`)[0] === f.fieldName ||
        ...
        if (aggregationOperators.some((fO) => logicalOperators.includes(operatorString.split(`${fO}_`)[1]))) {
            if (field.typeMeta.name === "String") {
                ...
                withStrs.push(`size(${variable}.${dbPropertyName}) AS ${hoistedVariable}`);
                aggregations.push(`${isShortest ? `min` : "max"}(${hoistedVariable}) ${createOperator(stringOperator)} $${paramName}`);
                return;
            }
            const isMin = operatorString.startsWith("MIN_");
            const [, opString] = operatorString.split(`${isMin ? `MIN` : "MAX"}_`);
            aggregations.push(` ${isMin ? `min` : "max"}(${variable}.${dbPropertyName}) ${createOperator(opString)} $${paramName}`);
            return;
        }
...
function createPredicate({ node, aggregation, context, chainStr, field, nodeVariable, edgeVariable, relationship, }) {
    const aggregations = [];
    let withStrs = [];
    let params = {};
...
function createAggregateWhereAndParams({ node, field, varName, chainStr, context, aggregation, relationship, }) {
    const cyphers = [];
...
exports.default = createAggregateWhereAndParams;
