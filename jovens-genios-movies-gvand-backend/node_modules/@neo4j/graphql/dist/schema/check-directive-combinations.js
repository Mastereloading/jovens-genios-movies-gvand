...
function checkDirectiveCombinations(directives = []) {
        ...
        if (invalidCombinations[directive.name.value]) {
            directives.forEach((d) => {
                if (invalidCombinations[directive.name.value].includes(d.name.value)) {
                    throw new Error(`Directive @${directive.name.value} cannot be used in combination with @${d.name.value}`);
                }
            });
        }
    });
}
exports.default = checkDirectiveCombinations;
//# sourceMappingURL=check-directive-combinations.js.map