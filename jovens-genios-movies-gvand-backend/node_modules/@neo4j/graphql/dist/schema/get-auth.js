...
const validOperations = ["CREATE", "READ", "UPDATE", "DELETE", "CONNECT", "DISCONNECT", "SUBSCRIBE"];
const validFields = [
    "operations",
    "AND",
    "OR",
    ...
    rules.value.values.forEach((rule) => {
        ...
        rule.fields.forEach((field) => {
            ...
            field.value.values.forEach((value) => {
                ...
                if (!validOperations.includes(value.value)) {
                    throw new Error(`auth rules rule operations operation invalid ${value.value}`);
                }
            });
        });
    });
    auth.rules = (0, graphql_1.valueFromASTUntyped)(rules.value);
    return auth;
}
